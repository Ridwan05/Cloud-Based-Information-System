...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('sales_records', function (Blueprint $table) {
            ...
            $table->decimal('total_revenue', 15, 2, true)
                ->comment('Amount received, in naira');

            $table->decimal('outstanding_balance', 15, 2, true)
                ->comment('Outstanding balance, in naira');

            $table->decimal('balance_payment', 15, 2, true)
                ->comment('Amount received as payment for debt, in naira');

            $table->decimal('cash_transfer_to_production', 15, 2, true)
                ->comment('Amount returned for production, in naira');

            $table->decimal('bank_deposit', 15, 2, true)
